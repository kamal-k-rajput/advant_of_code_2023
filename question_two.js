...
for (let index = 0; index < testInput.length; index++) {
  let min_requirment = { green: 0, blue: 0, red: 0 };

  const game = testInput[index];

  let temp_data = game.trim().split(":");
  let games = temp_data[1].trim().split(";");

  let gameId = +temp_data[0].trim().split(" ")[1];
  games.forEach((game, i) => {
    let one_game = game.trim().split(",");
    one_game = one_game.map((el) => el.trim());
    one_game.forEach((el) => {
      let [num, color] = el.trim().split(" ");
      if (min_requirment[color] < num) {
        min_requirment[color] = +num;
      }
    });
  });

  let i = 0;
  for (let key in available_balls) {
    if (available_balls[key] >= min_requirment[key]) {
      i++;
    }
    if (i === 3) {
      result += gameId;
...
function total_power_seciton_two(testInput) {
  let result = 0;
  for (let index = 0; index < testInput.length; index++) {
    let min_requirment = { green: 0, blue: 0, red: 0 };
    ...
    for (let key in min_requirment) {
      one_game_power *= +min_requirment[key];
    }
    console.log(result, "result");
    result += one_game_power;
  }
  return result;
}
...
total_power = total_power_seciton_two(section_two_input);
