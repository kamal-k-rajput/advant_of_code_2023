...
input.forEach((element) => {
  ...
  let string_array = element.split("");

  let initialtemp = false;
  let finaltemp = false;
  for (let i = 0; i < string_array.length; i++) {
    let first_character_ascii = element.charCodeAt(i);
    let last_character_ascii = element.charCodeAt(string_array.length - i - 1);

    if (
      first_character_ascii >= 48 &&
      first_character_ascii <= 57 &&
      !initialtemp
    ) {
      initialtemp = true;
      first_digit = string_array[i];
    }

    if (
      last_character_ascii >= 48 &&
      last_character_ascii <= 57 &&
      !finaltemp
    ) {
      finaltemp = true;
      second_digit = string_array[string_array.length - i - 1];
    }
    if (initialtemp && finaltemp) break;
  }
  digit_string = first_digit + "" + second_digit;
  sum += parseInt(digit_string);
});
...
let seciton_two_sum = 0;
let length = testInputTwo.length;
for (let i = 0; i < length; i++) {
  let str = testInputTwo[i];
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    for (let j = i; j < str.length; j++) {
      ...
      if (obj.hasOwnProperty(substr) && !first_get) {
        first_get = true;
        first_digit_ = obj[substr];
        break;
      }
    }
    let first_character_ascii = str.charCodeAt(i);
    if (
      first_character_ascii >= 48 &&
      first_character_ascii <= 57 &&
      !first_get
  ...
  for (let i = str.length - 1; i >= 0; i--) {
    ...
    for (let j = i; j >= 0; j--) {
      ...
      if (obj.hasOwnProperty(substr) && !last_get) {
        last_get = true;
        last_digit_ = obj[substr];
        break;
      }
    }
    let last_character_ascii = str.charCodeAt(i);
    if (last_character_ascii >= 48 && last_character_ascii <= 57 && !last_get) {
      last_get = true;
      last_digit_ = str[i];
    }
    if (last_get) break;
  }
  let digit = first_digit_ + "" + last_digit_;

  seciton_two_sum += +digit;
}
console.log(seciton_two_sum, "section_two_sum");
